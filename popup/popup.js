...
const templateRow = document.querySelector('.clone-me');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    browser.storage.local.get(['darkMode', 'ruleCount']).then(data => {
...
function initRows(storage) {
    ...
    const existingRules = rulesContainer.querySelectorAll('.rule:not(.clone-me)');
...
function deleteRow(event) {
    ...
    setTimeout(() => {
        ...
        const remainingRules = rulesContainer.querySelectorAll('.rule:not(.clone-me)');
...
function addRow(lhs = '', rhs = '') {
    ...
    row.classList.remove('clone-me', 'hidden');
    row.style.opacity = '0';
...
function makeDraggable(row) {
    ...
    dragHandle.addEventListener('mousedown', (e) => {
        ...
        const allRows = Array.from(rulesContainer.querySelectorAll('.rule:not(.clone-me)'));
...
function updateLocalStorage() {
    ...
    const rows = rulesContainer.querySelectorAll('.rule:not(.clone-me)');
